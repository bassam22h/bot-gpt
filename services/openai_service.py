...
def dialect_examples(dialect):
    examples = {
        "المغربية": """\
- استعمل كلمات مثل: "واخّا"، "بزاف"، "دابا"، "خويا"، "نعيقو".
- خليك خفيف الدم وكتبت بحيوية الشباب المغربي على السوشيال ميديا.
مثال: "دابا خوتي، واش عارفين شنو واقع؟ خاصنا نكونو مجموعين ونديرو البوز!"\
""",
        "المصرية": """\
- استعمل كلمات زي: "يلا بينا"، "جامد أوي"، "كده يعني"، "بص يا معلم".
- خليك فرفوش واكتب بطريقة عفوية زي الشباب على تويتر.
مثال: "بص يا صاحبي، اللي جاي تقيل جدًا، يلا ننجز!"\
""",
        "اليمنية": """\
- استخدم كلمات مثل: "شاص"، "دقدق"، "شوف كيف"، "معك خبر؟"، "باقي تمام؟"
- خليك طبيعي واكتب زي ما تتكلم مع أصحابك.
مثال: "عادك دريت؟ الدنيا قامت وما قعدت، شوف كيف الأخبار!"\
"""
    }
...
- استخدم أسلوبًا بسيطًا وغير رسمي.
    ...
    for attempt in range(max_retries):
        try:
            ...
            if platform == "تويتر":
                ...
                style_note = f"\nاكتب باللهجة {dialect} بأسلوب محلي شبابي.\n{dialect_examples(dialect)}" if dialect else ""
